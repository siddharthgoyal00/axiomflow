package com.axiomflow.axiomflow_backend.exception;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleBadCredentials(BadCredentialsException ex) {
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
            .body(new ErrorResponse(401, "Invalid credentials", ex.getMessage(), LocalDateTime.now()));
    }
    ...
    public ResponseEntity<ErrorResponse> handleGeneral(Exception ex) {
        ...
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
            .body(new ErrorResponse(500, "Internal server error", "Something went wrong", LocalDateTime.now()));
    }
}
